  ...
  Widget build(BuildContext context) {
    if (steps.isEmpty) {
      return const _JourneyError(
          message: 'لا توجد مراحل رحلة متاحة الآن. أعد المحاولة لاحقًا.');
    }
    final currentStep =
...
class _JourneyHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      constraints: BoxConstraints(minHeight: accessible ? 470 : 290),
      decoration: const BoxDecoration(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: accessible ? 20 : 6),
            Icon(
              Icons.mosque_rounded,
              size: accessible ? 76 : 92,
...
class _JourneyStatusCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final progress = overview.progress.clamp(0.0, 1.0).toDouble();
    final percent = '${(progress * 100).round()}%';
    final scaledBody = MediaQuery.textScalerOf(context).scale(16);
    final accessible = scaledBody >= 24;

    Widget indicator({required bool showLabels}) {
      return SizedBox(
        width: 104,
        height: 104,
        child: Stack(
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final currentStage = _MiniStage(
          label: 'المرحلة الحالية',
          title: currentStep.titleAr,
          subtitle: currentStep.descriptionAr,
          icon: Icons.health_and_safety_rounded,
        );
        final nextStage = _MiniStage(
          label: 'الخطوة التالية',
          title: overview.nextMilestoneAr
              .replaceFirst('الخطوة التالية:', '')
              .trim(),
          subtitle: overview.travelWindowAr,
          icon: Icons.chevron_left_rounded,
          gold: true,
        );

        final content = constraints.maxWidth < 430
            ? Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  currentStage,
            ...
            : Row(
                children: [
                  Expanded(child: currentStage),
                  Container(
                    width: 1,
                    height: 60,
                    color: const Color(0xFFECE5D8),
                  ),
                  Expanded(child: nextStage),
                ],
              );
        ...
        children: [
          Text(label,
              style: Theme.of(context).textTheme.labelMedium?.copyWith(
                  color: const Color(0xFF7A817B), fontWeight: FontWeight.w800)),
...
class _TimelineRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final active = step.status == JourneyStepStatus.current ||
        step.status == JourneyStepStatus.attention;
    final done = step.status == JourneyStepStatus.completed;
    final color = active
        ? MunasaknaTheme.haramGreen
        : done
            ? const Color(0xFF6BAA36)
            : const Color(0xFFCDD8D0);
    return InkWell(
      onTap: onTap,
          ...
          SizedBox(
            ...
            child: Column(
              children: [
                ...
                if (!isLast)
                  Container(
                      width: 2, height: 42, color: const Color(0xFFDDE5DF)),
              ],
            ),
          ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
            colors: [Color(0xFFFFF9EC), Color(0xFFFFFDF8)]),
        border: Border.all(
            color: MunasaknaTheme.kiswahGold.withValues(alpha: 0.26)),
      ),
      child: Row(
        children: [
          Container(
            ...
            height: 48,
            decoration: BoxDecoration(
                color: MunasaknaTheme.kiswahGold.withValues(alpha: 0.15),
                shape: BoxShape.circle),
                ...
                child: Row(
                  children: [
                    const Icon(Icons.check_circle_rounded,
                        color: MunasaknaTheme.haramGreen, size: 20),
                    const SizedBox(width: 8),
